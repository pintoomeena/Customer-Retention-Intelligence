...
def _add_domain_features(frame: pd.DataFrame, reference_frame: pd.DataFrame) -> pd.DataFrame:
    for column in MISSING_FLAG_SOURCE_COLUMNS:
    ...
    frame["total_call_events"] = (
        frame["received_calls"].fillna(0)
        + frame["outbound_calls"].fillna(0)
        + frame["inbound_calls"].fillna(0)
        + frame["peak_calls_in_out"].fillna(0)
        + frame["off_peak_calls_in_out"].fillna(0)
    )
    ...
    frame["customer_care_rate"] = frame["customer_care_calls"] / (frame["total_call_events"] + 1)
    frame["dropped_rate"] = frame["dropped_calls"] / (frame["total_call_events"] + 1)
    frame["blocked_rate"] = frame["blocked_calls"] / (frame["total_call_events"] + 1)
    frame["unanswered_rate"] = frame["unanswered_calls"] / (frame["total_call_events"] + 1)
    frame["retention_offer_rate"] = frame["retention_offers_accepted"] / (frame["retention_calls"] + 1)
    frame["active_sub_ratio"] = frame["active_subs"] / (frame["unique_subs"] + 1)
    frame["handset_model_density"] = frame["handset_models"] / (frame["handsets"] + 1)
    frame["minutes_per_month_service"] = frame["monthly_minutes"] / (frame["months_in_service"] + 1)
    frame["revenue_per_month_service"] = frame["monthly_revenue"] / (frame["months_in_service"] + 1)
    ...
    frame["equipment_age_bucket"] = pd.cut(
        frame["current_equipment_days"].fillna(-1),
        bins=[-2, 30, 90, 180, 365, 9999],
        labels=["unknown_or_new", "1_3m", "3_6m", "6_12m", "12m_plus"],
    ).astype(str)

    high_usage_threshold = reference_frame["monthly_minutes"].quantile(0.75)
    low_revenue_threshold = reference_frame["monthly_revenue"].quantile(0.25)
    frame["high_usage_low_revenue_flag"] = (
        (frame["monthly_minutes"] > high_usage_threshold)
        & (frame["monthly_revenue"] < low_revenue_threshold)
    ).astype(int)
    frame["retention_but_no_accept_flag"] = (
        (frame["retention_calls"] > 0) & (frame["retention_offers_accepted"] == 0)
    ).astype(int)
...
def prepare_cell2cell_frames(
    ...
    prepared_train = _add_domain_features(prepared_train, reference_frame=prepared_train)
    prepared_holdout = _add_domain_features(prepared_holdout, reference_frame=prepared_train)

    for frame in (prepared_train, prepared_holdout):
        frame["service_area_prefix"] = frame["service_area_grouped"].astype(str).str[:3]
        frame["credit_prizm_combo"] = (
            frame["credit_rating_grouped"].astype(str) + "__" + frame["prizm_code_grouped"].astype(str)
        )
        frame["marital_home_combo"] = (
            frame["marital_status"].astype(str) + "__" + frame["homeownership"].astype(str)
        )
